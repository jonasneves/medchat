...
def try_add_github_secret(token: str) -> bool:
    try:
        ...
        process = subprocess.Popen(
            ["gh", "secret", "set", "MEDCHAT_TUNNEL_TOKEN"],
...
def setup_tunnel(domain: str, subdomain: str, no_auto_secret: bool = False) -> None:
    ...
    if not no_auto_secret and try_add_github_secret(tunnel_token):
        print("Secret MEDCHAT_TUNNEL_TOKEN added to GitHub")
    else:
        print(f"\nAdd this token as GitHub secret MEDCHAT_TUNNEL_TOKEN:")
        print(tunnel_token)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Setup Cloudflare tunnel for MedChat")
    parser.add_argument("--domain", default="neevs.io", help="Domain (default: neevs.io)")
    parser.add_argument("--subdomain", default="medchat", help="Subdomain (default: medchat)")
